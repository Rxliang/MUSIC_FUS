            ...
            try:
                ...
                for data_key in self.data_keys:
                    if data.get(data_key) is not None:
                        tempt_env = data[data_key].flatten()
                ...
                if "FUSoff2.mat" in file:
                    filtered_data = tempt_env[10000:]
                    print("clippppp")
                else: 
    ...
    def plot_data(self, save_fig=False, fig_dir='plots'):
        ...
        for (date, session), group in grouped_data:
            ...
            for _, row in sorted_group.iterrows():
                ...
                current_index += len(filtered_data) + 20000#len(filtered_data)  # Leave a gap of 500 samples between each file
            # np.save(f"{date}_{session}_stitched_data.npy", stitched_data)
            # np.save(f"{date}_{session}_stitched_indices.npy", stitched_indices)
    ...
    def summarize_changes(self, window_size=1000, baseline_dir=None, save_fig=False, fig_dir='plots', palette='Set2',scale_data=False):
        ...
        for (date, session), group in grouped_data:
            ...
            for i in range(num_changes):
                ...
                main_data.append(tempt)
                # Label the data for main data
                main_labels.extend([f'{session}'] * (2 * window_size))
